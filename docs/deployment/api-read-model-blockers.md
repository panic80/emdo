...
composition now exist for all eight components. No process-memory or fixture
...
| `experience.finance-imports`          | Request-current, receipt-backed CSV/OFX preview and commit routes over forced-RLS plans, fingerprints, and immutable receipts; raw statement bytes never persist. | Fresh and upgrade PG17 migration/repository proof plus the exact-login finance readiness probe. Plan-retention scheduling remains separately uncomposed. | The Finance page keeps the source in memory, requires explicit preview/review/commit, and wipes it only after a correlated verified receipt. |
