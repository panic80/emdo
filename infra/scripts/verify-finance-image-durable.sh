...
cd "$repo_root"
proof_dir=$(mktemp -d "${TMPDIR:-/tmp}/emdo-image-durable.XXXXXX")
name="emdo-image-durable-$$"
cleanup() { docker rm -f -v "$name-client" "$name-helper" "$name-pg" >/dev/null 2>&1 || true; docker volume rm "$name-socket" >/dev/null 2>&1 || true; docker network rm "$name-network" >/dev/null 2>&1 || true; rm -rf "$proof_dir"; }
trap cleanup EXIT
apps/worker/node_modules/.bin/esbuild apps/worker/src/finance-image-durable.acceptance.ts --bundle --platform=node --format=esm --external:pdf-parse --external:pdfjs-dist --banner:js="import { createRequire } from 'node:module'; const require=createRequire(import.meta.url);" --outfile="$proof_dir/client.mjs"
cp infra/finance-ocr/acceptance/statement.png "$proof_dir/statement.png"
chmod 755 "$proof_dir"
chmod 644 "$proof_dir/client.mjs" "$proof_dir/statement.png"
...
docker inspect "$name-helper" --format 'helper network={{.HostConfig.NetworkMode}} readonly={{.HostConfig.ReadonlyRootfs}} user={{.Config.User}} caps={{json .HostConfig.CapDrop}}'
docker run --platform linux/amd64 --rm --name "$name-client" --network "$name-network" --read-only --cap-drop ALL --security-opt no-new-privileges:true --user 10002:10004 --env FINANCE_V2_TEST_DATABASE_URL=postgresql://postgres@postgres/emdo_image_acceptance --volume "$name-socket:/run/emdo/finance-ocr:ro" --volume "$proof_dir:/proof:ro" --entrypoint /usr/local/bin/node "$image_id" /proof/client.mjs
