...
google_reconciliation_capacity_lock_fd=''
google_reconciliation_operation_lock_fd=''
acquire_host_lock /var/lib/emdo/locks/capacity.lock google_reconciliation_capacity_lock_fd
acquire_host_lock /var/lib/emdo/locks/production-mutation.lock google_reconciliation_operation_lock_fd
: "$google_reconciliation_capacity_lock_fd" "$google_reconciliation_operation_lock_fd"

assert_deployed_release_lock "$PRODUCTION_STATE_DIR/current.env"
google_reconciliation_release_root="$(cd -- "$SCRIPT_DIR/../.." && pwd -P)"
[[ "$google_reconciliation_release_root" == "$IMAGE_LOCK_DEPLOYED_RELEASE_DIR" ]] ||
  die 'Google OAuth disconnect reconciliation must execute the assets bound to current production state'
export_digest_lock
...
log 'Google OAuth disconnect reconciliation completed through the provider-free bounded runner'
