...
const environment = {
  ...
  LANG: 'C', LC_ALL: 'C', MAGICK_CONFIGURE_PATH: '/etc/ImageMagick-6',
  TESSDATA_PREFIX: '/usr/share/tesseract-ocr/5/tessdata',
};
// Only a stale socket may be removed. Never follow or replace a caller's file.
try {
  if (!(await lstat(path)).isSocket()) throw new Error('ocr-socket-path-invalid');
  await unlink(path);
} catch (error) {
...
const server = createServer({ allowHalfOpen: true }, (socket) => {
  ...
  const child = spawn(executable, [], {
    env: environment, cwd: '/tmp', detached: true, stdio: ['pipe', 'pipe', 'ignore'],
  });
  let received = 0, sent = 0, finished = false;
  const stop = () => {
    if (finished) return;
    finished = true;
    clearTimeout(timer);
  ...
  socket.on('data', (bytes) => { received += bytes.length; if (received > maximumBytes) stop(); });
...
for (const signal of ['SIGTERM', 'SIGINT']) {
  process.on(signal, () => { active?.(); server.close(); });
}
