...
describe('Finance staging acceptance failure diagnostic', () => {
  ...
  const resumedRunFailureOutcomes = [
    'approval-resume-binding-invalid',
    'approval-resume-failed',
    ...resumedRunRunnerCodes.map((code) => `runner-${code}`),
    'unrecognized',
    'invalid',
  ];
  const acceptedFailures = [
    ...
    ...resumedRunFailureOutcomes.map((outcome) =>
      stagingFailure(
        `guarded-review-commit:resumed-run-failed outcome=${outcome}`,
      ),
    ),
  ];
  ...
  it('fails closed for altered, duplicate, missing, oversized, and symlinked markers', async () => {
    ...
    try {
      for (const input of [
        ...
        `${stagingFailure('qna-and-isolation:turn outcome=private-response')}\n`,
        `${stagingFailure('qna-and-isolation:citations:private-source')}\n`,
        `${stagingFailure('qna-and-isolation:evidence-private')}\n`,
        `${stagingFailure('qna-and-isolation:member-list extra=private-member')}\n`,
        `${stagingFailure('qna-and-isolation:cross-user-private')}\n`,
        `${stagingFailure('safe-write-and-handoff:direct-turn-private')}\n`,
        `${stagingFailure('safe-write-and-handoff:sse-terminal outcome=private-response')}\n`,
        `${stagingFailure('safe-write-and-handoff:completed-output-validation-private')}\n`,
        `${stagingFailure('safe-write-and-handoff:finance-experience-readback extra=private-path')}\n`,
        `${stagingFailure('safe-write-and-handoff:protected-handoff-precondition-private')}\n`,
