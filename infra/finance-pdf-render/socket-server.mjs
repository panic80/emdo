...
try {
  if (!(await lstat(path)).isSocket())
    throw Error('pdf-render-socket-path-invalid');
  await unlink(path);
} catch (error) {
...
const server = createServer({ allowHalfOpen: true }, (socket) => {
  ...
  socket.on('data', (bytes) => {
    ...
    if (expected === undefined && request.length >= magic.length + 4) {
      ...
      if (
        !request.subarray(0, magic.length).equals(magic) ||
        headerLength < 1 ||
        headerLength > 2048
      ) {
        stop();
      ...
      if (request.length >= magic.length + 4 + headerLength) {
        try {
          const header = JSON.parse(
            request.subarray(magic.length + 4, magic.length + 4 + headerLength),
          );
          if (
            !Number.isInteger(header.byteLength) ||
            header.byteLength < 1 ||
            header.byteLength > 2 * 1024 * 1024
          )
            throw Error('length');
