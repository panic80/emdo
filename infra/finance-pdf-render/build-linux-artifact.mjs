...
if (!image)
  throw new Error(
    'usage: build-linux-artifact.mjs <output> <locally-installed-linux-dependency-image>',
  );
...
execFileSync(
  process.execPath,
  [join(dirname(fileURLToPath(import.meta.url)), 'build.mjs'), output],
  { stdio: 'inherit' },
);
...
try {
  for (const [name, path] of paths) {
    if (
      !/^(@napi-rs\/canvas(?:-[a-z0-9-]+)?|pdfjs-dist)$/.test(name) ||
      !path.startsWith('/app/node_modules/') ||
      !path.endsWith('/package.json')
    )
      throw Error('dependency-path-invalid');
...
await writeFile(
  join(output, 'linux-provenance.json'),
  JSON.stringify(
    {
      dependencyImage: imageId,
      platform: `linux/${inspected.Architecture}`,
...
console.log(
  `Offline Linux artifact ready: ${output} (${inspected.Architecture})`,
);
