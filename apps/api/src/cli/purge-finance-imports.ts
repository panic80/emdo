...
const RETENTION_LOGIN = 'emdo_finance_import_retention_login';
const RETENTION_ROLE = 'emdo_finance_import_retention';
...
const executePurge = async (input: {
  ...
  try {
    ...
    const result = await client.query(
      'select emdo.purge_expired_finance_import_plans($1::integer) as purged',
      [input.limit],
    );
  ...
  if (!succeeded) {
    throw new Error('Finance import retention failed');
  }
  return purged;
};

export const runFinanceImportRetentionCommand = async (input: {
  ...
  const parsed = RetentionConfigurationSchema.safeParse({
    databaseUrl: input.environment.EMDO_FINANCE_IMPORT_RETENTION_DATABASE_URL,
    deploymentEnvironment: input.environment.EMDO_ENVIRONMENT,
    limit: input.environment.EMDO_FINANCE_IMPORT_RETENTION_LIMIT,
  });
  if (
    input.argv.length !== 1 ||
    input.argv[0] !== '--purge-expired-plans' ||
    !parsed.success
  ) {
    throw new Error('Finance import retention configuration is invalid');
  }
  const purged = await executePurge({
...
if (
  ...
  void runFinanceImportRetentionCommand({
    argv: process.argv.slice(2),
    environment: process.env,
  })
    .then((result) => process.stdout.write(`${JSON.stringify(result)}\n`))
    .catch(() => {
      process.stderr.write('Finance import retention failed.\n');
      process.exitCode = 1;
    });
}
