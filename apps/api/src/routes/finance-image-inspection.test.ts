...
const base = `/api/v2/finance/books/${uuid(5)}/evidence/${uuid(6)}/image-inspection`;
async function fixture() {
  ...
  const inspection = {
    ...
    sourceDigest: digest,
    wordInventoryDigest: createHash('sha256').update('[]').digest('hex'),
    facts: {
      status: 'no-text',
...
describe('saved image inspection endpoint', () => {
  ...
  it.each([
    ...
    try {
      ...
      if (kind === 'wrong-source') {
        ...
        f.inspection.facts.sourceDigest = 'f'.repeat(64);
      }
      if (kind === 'wrong-inventory')
        f.inspection.wordInventoryDigest = 'f'.repeat(64);
