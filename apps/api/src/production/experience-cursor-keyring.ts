...
const ExperienceCursorKeyringSchema = z.strictObject({
...
const invalidKeyring = (): Error =>
  new Error('api-experience-cursor-keyring-invalid');
...
export interface ProductionExperienceCursorKeyring {
  readonly current: ExperienceQueryCursorHmacKey;
  readonly previous: readonly ExperienceQueryCursorPreviousHmacKey[];
}

export const parseProductionExperienceCursorKeyring = (
  encoded: string,
): ProductionExperienceCursorKeyring => {
  ...
  try {
    ...
    const keyring = ExperienceCursorKeyringSchema.parse(raw);
    const keyIds = [
