...
const VisualProofKeyringSchema = z.strictObject({
...
const invalidKeyring = (): Error =>
  new Error('api-visual-proof-keyring-invalid');
...
/**
 * Decodes the private production key ring, constructs the codec, and wipes
 * every temporary decoded secret. The codec owns independent secret copies.
 */
export const createProductionVisualProofTokenCodec = (
  encoded: string,
  ...
  try {
    ...
    const keyring = VisualProofKeyringSchema.parse(raw);
    const keyIds = [
    ...
    const current: VisualDecisionProofHmacKey = {
      keyId: keyring.current.keyId,
      secret: currentSecret,
    };
    const previous: VisualDecisionProofPreviousHmacKey[] = keyring.previous.map(
      (candidate) => {
    ...
    return new VisualDecisionProofTokenCodec({
      current,
      previous,
