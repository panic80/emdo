...
test('reviewed XLSX candidate requires a separate approval before import', async ({
  page,
}, testInfo) => {
  const errors: string[] = [];
  ...
  await page.route('**/api/v2/finance/books**', async (route) => {
    ...
    if (req.method() === 'POST' && path.endsWith('/report-mappings')) {
    ...
    const result = path.endsWith('/books')
      ...
      : path.endsWith('/evidence')
        ? {
            documents: [
              { id: evidenceId, filename: 'reviewed.xlsx', format: 'xlsx' },
            ],
            nextOffset: null,
          }
  ...
  await page
    ...
    .click();
  await page
    .getByRole('button', { name: 'Example · Activity · v1 · candidate' })
    .click();
  await expect(
    page.getByRole('table', { name: 'Original example rows' }),
  ).toBeVisible();
  await expect(
    page.getByRole('button', { name: 'Choose account for reuse' }),
  ).toHaveCount(0);
  await page
    ...
    .click();
  await page
    .getByLabel('Are amounts CAD?')
    .fill('The original currency column confirms CAD.');
  await page
    .getByLabel('Source review notes')
    .fill('Checked the complete data range against the original.');
  await page.getByLabel(/I checked the range, currency/).check();
  await page.screenshot({
    path: '/tmp/emdo-xlsx-review-form-desktop.png',
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: '/tmp/emdo-xlsx-review-form-mobile.png',
    fullPage: true,
  });
  await page
    .getByRole('button', { name: 'Save revised XLSX candidate' })
    .click();
  await expect(
  ...
  await page
    ...
    .fill('Confirmed the source-derived values and mapping.');
  ...
  await expectNoSeriousAccessibilityViolations(page);
  await page.screenshot({
    path: testInfo.outputPath('xlsx-review-desktop.png'),
    fullPage: true,
  });
  await page.setViewportSize({ width: 390, height: 844 });
  expect(
  ...
  await page.screenshot({
    path: testInfo.outputPath('xlsx-review-mobile.png'),
    fullPage: true,
  });
