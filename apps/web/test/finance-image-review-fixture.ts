...
export function imageReviewFixture() {
  ...
  const run = standardization.createRun('needs-review', 'png');
  run.filename = 'image-statement.png';
  ...
  const original = {
    ...
    format: 'png' as const,
    sourceBase64: imageReviewBytes.toString('base64'),
  };
  const state = {
  ...
  const makeMapping = (
    payload: ReturnType<typeof SaveReviewedFinanceImageMappingSchema.parse>,
  ) => {
    const result = extractReviewedFinanceImageTable(
      imageReviewBytes,
      payload.proposal.definition.imageSelection,
      {
        standardizationRunId: run.id,
        extractionRevision: 1,
        extractionDigest: inspection.extractionDigest,
        factsJson: JSON.stringify(inspection.facts),
      },
      {
  ...
  function handle(
    ...
    if (method === 'POST') {
      ...
      if (path === `${base}/report-mappings`) {
        ...
        const payload = SaveReviewedFinanceImageMappingSchema.parse(body);
    ...
    if (path.endsWith('/image-inspection')) {
      if (
      ...
      return {
        ...
        json: state.inspectionStatus === 200 ? inspection : {},
      };
    }
