...
-- Production fixed executor: revision-bound delivery only, never the legacy claim entry point.
GRANT EXECUTE ON FUNCTION emdo.claim_finance_automation_delivery(uuid,integer),emdo.settle_finance_automation_run(uuid,integer,uuid,text,uuid) TO emdo_worker_executor;
