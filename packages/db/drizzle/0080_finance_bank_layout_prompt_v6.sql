-- Admit reviewed split-column bank mappings and explicit statement years in prompt v6.
-- Keep all prior prompt versions readable and preserve PDF reservation bounds.
...
--> statement-breakpoint
ALTER TABLE emdo.finance_standardization_spend ADD CONSTRAINT standardization_spend_bounds CHECK ("emdo"."finance_standardization_spend"."attempt" between 1 and 3 and "emdo"."finance_standardization_spend"."input_token_ceiling" between 1 and (case when "emdo"."finance_standardization_spend"."lineage"->>'promptVersion' IN ('finance-standardization-proposal.v5','finance-standardization-proposal.v6') and "emdo"."finance_standardization_spend"."lineage"->'promptProjection'->>'kind'='pdf-text.v1' then 64000 else 20000 end) and "emdo"."finance_standardization_spend"."output_token_ceiling" between 1 and 4000 and "emdo"."finance_standardization_spend"."reserved_cad_minor">0 and ("emdo"."finance_standardization_spend"."actual_cad_minor" is null or "emdo"."finance_standardization_spend"."actual_cad_minor">=0) and "emdo"."finance_standardization_spend"."status" in ('reserved','completed','not-sent','indeterminate') and length("emdo"."finance_standardization_spend"."request_key") between 1 and 200 and length("emdo"."finance_standardization_spend"."pricing_version") between 1 and 128);
